...
The versioned [label policy](../corpus/labels.json) records every confirmed positive, individually reviewed negative, provisional-negative batch, and unlabeled batch. Per-thread tables and machine-readable feature exports are generated research artifacts and are intentionally not tracked. Only the two supplied files—not their entire source thread—were analyzed for the supplemental set. A later external-review proposal study is documented separately in [PROPOSAL_EVALUATION.md](PROPOSAL_EVALUATION.md).

Thread `336272252` was retained as a separate corpus batch, and all 349 video attachments had decodable audio. The user later labeled `17884174673280229863.mp4` and `17884274747240014140.mp4` as screamers; the remaining 347 clips are still unlabeled rather than presumed negative, so they are excluded from labeled evaluation.
